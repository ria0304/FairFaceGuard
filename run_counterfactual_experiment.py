...
from src.reports.week3_metric_report import (
    build_week3_report,
    print_report as print_week3_report,
    save_report as save_week3_report,
    save_report_csv as save_week3_report_csv,
)
...
def stage_annotate(data_root: str) -> None:
    ...
    logger.info("Wrote %d annotations to %s", len(df), out_path)
...
def stage_augment(data_root: str) -> dict:
    ...
    for _, row in annotations.iterrows():
        ...
        if not cf.accepted:
            logger.warning("[REJECTED] %s: %s", face_id, cf.reject_reasons)

    logger.info("Wrote counterfactual sets to %s", out_root)
...
def stage_baseline(data_root: str, epochs: int = 20) -> BaselineDeepfakeDetector:
    ...
    save_week3_report_csv(week3_report, os.path.join(data_root, "week3_metric_report.csv"))
...
def stage_report(data_root: str, subgroup_results: dict, week4_results: dict) -> None:
    ...
    logger.info(report["headline_conclusion"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_level", default="INFO", help="DEBUG, INFO, WARNING, ERROR")
